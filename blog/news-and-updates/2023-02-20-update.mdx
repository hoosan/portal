...
新しいMotoko アップデートポストの時間です！

言語チームからの前回の更新記事を見逃した方は、[こちら](https://forum.dfinity.org/t/new-candid-version-and-catching-send-failures-motoko-updates/18410)
をご覧ください！前回の
では、メッセージ送信時のエラーの検出と Candid の新バージョンについてお話しました。

今日はガベージコレクションのインクリメンタル化とVSCodeエクステンションのパフォーマンス改善についてです！ボーナスとして
GitHub copilot/GPT3 の使い方についてMotoko 。

## インクリメンタルGC：グローバルR\&Dでのプレゼンテーション！

![Header image](../_assets/garbage-truck.png)

FIXMETAGLUC @FIXMETAGLUC は、Motoko のための新しいガベージコレクタをまとめるために、過去何ヶ月も懸命に取り組んできました。この
インクリメンタル GC は、より熱心に未使用オブジェクトを収集し、
、ヒープを完全に掃引する GC の実行を回避することで、言語のスケーラビリティ問題を緩和するように設計されています。

そして明日、Luc は今月のグローバル R\&D 公開プレゼンテーションで、このプロジェクトについてライブで発表します！

いつ？2023年2月22日 / 17:30 CET / 8:30 PDT
場所:
https://dfinity.us16.list-manage.com/track/click?u=33c727489e01ff5b6e1fb6cc6\&id=ce77db3b84\&e=8cf7869fe3

登録がお済みでない方は、上記のリンクからご登録ください！また、このミーティングは録画され、YouTuneチャンネル
にアップされます。

https://www.youtube.com/@DFINITY

この機能に関する詳細なディスカッションをご覧になりたい方は、
[PRを](https://github.com/dfinity/motoko/pull/3756)ご覧ください。
...
VSCodeエクステンションは、大規模なプロジェクト（Motoko ）において約10倍から100倍高速に動作するようになりました！

パフォーマンスの向上は、タイプチェックを行うファイルの選択をよりスマートにしたことによるものです。ワークスペース内のすべての
ファイルをタイプチェックする代わりに、開いているファイルと dfx.jsoncanister エントリポイントのみをチェックするようになりました (
インポートされたすべてのファイルと一緒に)。これはまた、大規模なプロジェクトで VS Code と言語サーバー間で渡される
メッセージの量を減らすために、他の最適化（診断のキャッシュなど）と重ねられました。

この最適化は実験的な機能としてリリースされていますので、バグが発生したらお知らせください！

## おまけ: GitHub Copilot xMotoko

最近、
@FIXMEKYLEの提案で、GitHub CopilotというGitHubのジェネレーティブAIコーディングツールで遊んでいます。このツールはGPT3のラッパーで、最近有名になったChatGPTの前身モデルです。コードを入力すると、
Copilotがあなたの意図を予測し、
、シームレスに受け入れたり拒否したりできる構文を押し付けがましくなく提案してくれます。私は、Motoko 、
のような新しい言語であっても、モデルの学習データが少ないと思われる場合でも、開発速度を向上させるのに非常に役立つことがわかりました。
のような新しい言語では、モデルの学習データが少ないと思われます。

ツールの検索はこちら：https://github.com/features/copilot

オープンソースでないプロジェクトの場合、ツールがあなたのリポジトリ/コードのデータを収集する可能性があることに注意してください。
