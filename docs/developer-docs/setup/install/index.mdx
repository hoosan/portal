...
<p>Windowsでは、<code>dfx</code> のネイティブ・サポートはありません。しかし、Windows Subsystem for
Linux (WSL)をインストールすることで、<code>dfx</code> を Windows システム上でも実行することができます。</p>
<h2 id="installing-wsl">WSL 2 のインストール</h2>
<p><a
href="https://docs.microsoft.com/en-us/windows/wsl/install">Windows Subsystem for Linux</a>のインストールについては、Microsoft の指示に従ってください。
