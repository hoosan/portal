...
神経系パラメータは、特定のSNSの設定を定義します。

すべてのSNSは同じアップグレードパスに沿ってアップグレードされるため\[アーキテクチャを参照\]、SNSがガバナンスのルールやトークノミクスなどを必要に応じてカスタマイズすることができます。

## `ManageNervousSystemParameters` 提案

提案`ManageNervousSystemParameters` は、SNSコミュニティがこれらのパラメータをいつでも調整できるようにするものです。
