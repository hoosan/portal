...
これでローカルな Bitcoin ネットワークが構築できたので Bitcoin Dapps の開発とローカルでのテストを始める準備ができています。
...
`bitcoind`と同じディレクトリで、以下のコマンドを発行してブロックをマイニングすることができます。
...
:::note
マイニングした BTC はあなたのローカル Bitcoin ネットワーク内でのみ有効であり、他の場所で使用したり、使ったりすることはできません。
:::

### coinbase maturity

前ステップで、あなたは1つのブロックをマイニングし、そのプロセスであなたの Canister に 50 BTC の報酬が与えられました。
...
上記のコマンドはトランザクションを作成し、ローカルの Bitcoin ノードに送出します。
...
ローカルにある Bitcoin のステートをすべて削除して、一からやり直すとよいことがしばしばあります。そのためには：
