...
ICP トークンを含むトランザクションでは、ネットワークが署名付きトランザクションを受け取る24時間以内に署名なしトランザクションが作成されなければなりません。
...
- `mempool` API を使用しないでください。私たちの実装は空のスタブです。
...
- Mint は秘匿された操作です。現在、Rosetta API コールによる Burn はサポートしていません。
